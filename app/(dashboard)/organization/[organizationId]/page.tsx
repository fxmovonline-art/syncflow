...
export default async function OrganizationDashboardPage({
  ...
  if (currentOrgId !== organizationId) {
    // User is trying to access an organization they're not a member of
    redirect("/dashboard");
  }
  ...
  // Fetch boards using the organization ID from URL params (not Clerk)
  const boards = await db.board.findMany({
    where: {
      orgId: organizationId, // Use URL param, not Clerk's orgId
    },
    ...
    <div className="flex flex-1 flex-col bg-zinc-100 dark:bg-zinc-950">
      <div className="border-b border-zinc-200 bg-zinc-50 dark:border-white/10 dark:bg-zinc-900">
        <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-5 py-6 sm:px-8 lg:px-10">
          <Link
            href="/dashboard"
            className="inline-flex w-fit items-center gap-2 rounded-full border border-zinc-900/10 bg-white/80 px-3 py-1.5 text-sm font-semibold text-zinc-700 shadow-sm transition hover:bg-white hover:text-zinc-950 dark:border-white/15 dark:bg-white/10 dark:text-white dark:hover:bg-white/20"
          >
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <p className="max-w-2xl text-base leading-7 text-slate-700 dark:text-slate-100/85">
                  Welcome back, {displayName}. Keep every team board organized,
                  visible, and ready for the next sprint.
                </p>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-zinc-900">
              ...
              <div className="mt-4 space-y-3">
                <div className="flex items-center justify-between rounded-xl bg-zinc-100 px-3 py-2 dark:bg-zinc-800">
                  ...
                  <span className="text-sm font-semibold text-zinc-900 dark:text-white">Team editable</span>
                </div>
                <div className="flex items-center justify-between rounded-xl bg-zinc-100 px-3 py-2 dark:bg-zinc-800">
                  <span className="inline-flex items-center gap-2 text-sm font-medium text-zinc-700 dark:text-zinc-200">
                  ...
                  <span className="text-sm font-semibold text-zinc-900 dark:text-white">{latestBoardDate}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-2xl border border-emerald-300 bg-[linear-gradient(135deg,#047857,#059669_45%,#34d399)] p-4 text-white shadow-sm">
              ...
              <p className="mt-2 text-xs text-emerald-100/90">Across organization</p>
            </div>
            <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-zinc-900">
              <div className="flex items-center justify-between">
              ...
              <p className="mt-3 text-4xl font-semibold leading-none text-zinc-900 dark:text-white">{boardsThisMonth}</p>
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-zinc-900">
              ...
              <p className="mt-3 text-4xl font-semibold leading-none text-zinc-900 dark:text-white">{boardsWithCovers}</p>
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-zinc-900">
              ...
              <p className="mt-3 text-4xl font-semibold leading-none text-zinc-900 dark:text-white">{boardsWithoutCovers}</p>
        ...
        <div className="mx-auto flex w-full max-w-7xl flex-1 px-5 py-8 sm:px-8 lg:px-10">
          <div className="grid min-h-[460px] w-full place-items-center rounded-lg border border-dashed border-emerald-300 bg-white/90 p-8 text-center shadow-lg shadow-emerald-900/5 dark:border-emerald-300/30 dark:bg-white/[0.06] dark:shadow-black/30">
            <div className="max-w-md">
              <div className="mx-auto mb-6 grid size-16 place-items-center rounded-lg bg-[linear-gradient(135deg,#059669,#0284c7,#c026d3)] text-white shadow-lg shadow-sky-900/20">
                <LayoutDashboard className="size-8" />
              </div>
              <h2 className="text-2xl font-semibold tracking-tight text-slate-950 dark:text-white">
                Build your first shared board
              </h2>
              <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
                Give the team one clean place to plan work, track priorities,
                and move ideas from discussion to delivery.
              </p>
              <CreateBoardModal>
                <Button
                  size="lg"
                  className="mt-7 bg-slate-950 text-white hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200"
                >
          ...
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                Fast access to every active organization workspace.
              </p>
            </div>
            <CreateBoardModal>
              <Button className="rounded-full bg-emerald-700 px-4 text-white hover:bg-emerald-600 dark:bg-emerald-500 dark:text-zinc-950 dark:hover:bg-emerald-400">
                <Plus className="size-4" />
                New board
              </Button>
            </CreateBoardModal>
            ...
            <CreateBoardModal>
              <button className="group flex min-h-52 cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-cyan-300 bg-cyan-50 p-6 text-center shadow-sm shadow-cyan-900/5 transition hover:-translate-y-0.5 hover:border-cyan-500 hover:bg-white hover:shadow-md dark:border-cyan-300/30 dark:bg-cyan-300/10 dark:hover:border-cyan-200/70 dark:hover:bg-cyan-300/15">
                ...
                <p className="mt-1 max-w-48 text-sm text-slate-600 dark:text-slate-300">
                  Start a clean space for a launch, sprint, or client project.
                </p>
              </button>
            </CreateBoardModal>
